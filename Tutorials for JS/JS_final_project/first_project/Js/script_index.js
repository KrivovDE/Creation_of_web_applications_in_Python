...
(function loadBlockFood(){
    for(let i = 0;i < 3;i++){
        ...
        $.ajax({
            url: 'https://api.spoonacular.com/recipes/random?apiKey=9c0472f7abaa406c840c34e684262489',
            dataType: 'json'
        }).done(function (data){
...
$('.searchRecept button').click(function (){
    for (let i = 0; i < 3;i++){
        ajaxRequest(`https://api.spoonacular.com/recipes/search?` +
            `apiKey=9c0472f7abaa406c840c34e684262489&number=3&query=${$('.searchRecept input').val()}`,i)
    }
})
