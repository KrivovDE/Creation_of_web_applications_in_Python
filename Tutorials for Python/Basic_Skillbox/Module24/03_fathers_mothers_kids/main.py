...
class Parent:
    ...
    def who_am_i(self):
        print(
            "Меня зовут {name}, мне {age} годика\nУ меня есть дети:".format(
                name=self.name,
                age=self.age,
            ),
        )
    ...
    def child_create(
        ...
        child_name: str,
        child_age: int = 0,
        child_status: str = "Орёт",
        child_hunger: bool = True,
    ):
...
class Child:
    ...
    def __init__(
        ...
        if patent_age - 16 > child_age:
            ...
            raise ValueError(
                "Разница возрастов между ребёнком и родителем"
                " не может быть меньше 16 лет",
            )

    def child_info(self):
        print(
            "Имя ребёнка - {child_name}, возраст - {child_age}, "
            "состояние - {child_status}, хочет есть {child_hunger}".format(
                child_name=self.name,
                child_age=self.age,
                child_status=self.status,
                child_hunger=self.hunger,
            ),
        )
