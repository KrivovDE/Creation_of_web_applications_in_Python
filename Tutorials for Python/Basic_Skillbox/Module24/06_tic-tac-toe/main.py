...
class Board:
    ...
    def __init__(self, x_size: int = 3, y_size: int = 3):
        self.board_list = [[Cell(column, row, column + row * x_size) for column in range(x_size)]
...
class Player():
    ...
    def move_check(self, index: int):
        try:
            ...
            if isinstance(index, int) and index < (len(self.board[0]) * len(self.board)):
...
class Game:
    ...
    def win_check(self, player):
        ...
        if self.move_counter < 5:
            ...
            combination = [''.join([self.board_list[y][x].busy for x in range(
                len(self.board_list[y]))]) for y in range(len(self.board_list))]
            combination.extend([''.join([self.board_list[x][y].busy
                                         for x in range(len(self.board_list[y]))])
                for y in range(len(self.board_list))])
            combination.extend([''.join([self.board_list[0][0].busy,
                                         self.board_list[1][1].busy,
                                         self.board_list[2][2].busy]),
                 ''.join([self.board_list[2][0].busy,
                          self.board_list[1][1].busy,
    ...
    def lets_play(self, active_player):
        ...
        for _ in range(9):
            print('Ходит -', active_player.name, 'ставит -',
                  active_player.sign)
            self.make_move(active_player)
            if self.win_check(active_player):
                print('Игрок {} победил'.format(active_player.name))
                active_player.win_counter += 1
                print('Счёт - {} : {}\n'.format(
                    self.player_1.win_counter, self.player_2.win_counter))
                break
            else:
                active_player = self.player_2 if active_player == self.player_1 else self.player_1
        else:
            print('Ничья')
...
while again:
    ...
    again = True if input('Для продолжения нажмите - 1, другая клавиша - Выход: ') == '1' \
        else False
else:
    print('End Game')