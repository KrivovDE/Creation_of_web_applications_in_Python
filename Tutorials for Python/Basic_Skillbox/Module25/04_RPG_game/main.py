...
def one_year_of_war():
    ...
    tank = Tank("Пётр")
    attacker = Attacker("Ольга")
    second_attacker = Attacker("Траур")
    ...
    if (
        sum([isinstance(hero, (MonsterHunter, MonsterBerserk)) for hero in good_team])
        > 1
    ):
    ...
    for day in range(1, 366):
        ...
        if day % 2 == 0 and len(evil_team) < 4:
            newborn_evils = [
                MonsterBerserk("Берсерк " + random.choice(evil_names)),
