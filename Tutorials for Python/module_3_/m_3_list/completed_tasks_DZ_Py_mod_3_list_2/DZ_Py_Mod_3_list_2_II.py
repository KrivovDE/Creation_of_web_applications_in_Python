...
print(f'Кол-во цифр 5 при первом объединении: {lst_first_second.count(5)}\n'
...
films = ['Крепкий орешек', 'Назад в будущее', 'Таксист', 'Леон',
         'Богемскаярапсодия', 'Город грехов', 'Мементо', 'Отступники',
         'Деревня']
lst_films = []
try:
    for i in range(int(input('Сколько фильмов хотите добавить? '))):
        choice = input('Введите название фильма: ').capitalize()
        if choice in films and choice not in lst_films:       #БЫЛО БЫ УДОБНЕЕ (choice in films and not lst_films)
            lst_films.append(choice)                              #но почему то не получается так возможно записать?
...
try:
    ...
    if title[0] in spec_sim:
        raise ValueError('Ошибка: название начинается на один из '
                         'специальных символов.')
    elif title.endswith((end[0], end[1])):
        print('Файл назван верно.')
    else:
        raise ValueError('Ошибка: неверное расширение файла. Ожидалось '
                         '.txt или .docx.')
...
string, new_string = input('Введите строку: ') + ' ', ''    #Хотелось бы увидеть более оптимальное решение
cnt = 1
